...
public function __construct()
 ...
 public function payment(Request $request)
 {
		...
		if(!empty($plano) && $plano->interval == 0){

		return $this->emitirBoletoAvulso($plano, $request['id'], $request['cpf'], $request['vencimento'] );
		...
		$body = [
 		...
 		try {
	 		...
	 		if ($pay_charge["code"] == 200) {
	            ...
	            if(!$anunciante->verified || $anunciante->verified == 0 || $anunciante->verified == null ) {
	...
	public function cartao(Request $request, $anunciante_id, $plano_id)
	{
		...
		if($anunciante->verified == 1){
			return redirect()->route('anunciante.login')->with('status', 'Tu;do certo! Você deve receber um email para cadastrar sua senha em breve. Obrigado');
		}
	...
	public function credito(Request $request)
	{
		...
		if(!empty($plano) && $plano->interval == 0){
			...
			return $this->emitirCreditoAvulso($plano, $cliente, $billingAddress );
	    ...
	    try {
	            ...
	            if($pay_charge['code']  == 200 ){
					...
					if(!$anunciante->verified || $anunciante->verified == 0 ) {
		            	$anunciante->verified = 1;  
	        			event(new CadastrarSenhaAnunciante($anunciante));
		        	}
...
/*================= Metodos para Pagamentos Pre Pagos =======================*/

	public function emitirBoletoAvulso($plano, $user, $cpf, $validade )
	{
		
		$anunciante = Anunciante::find($user);
		$telefone = preg_replace("/[^0-9]/", "", $anunciante->celular);
	...
	public function emitirCreditoAvulso($plano, $cliente, $billingAddress )
	{
    	...
    	if ($charge["code"] == 200) {
			...
			$customer = [
	            'name' => $cliente['name'],
	            'cpf' => $cliente['cpf'] ,
	            'phone_number' => preg_replace("/[^0-9]/", "", $cliente['telefone']),
	            'email' => $cliente['email'],
	            'birth' => $cliente['birth']
	        ];
